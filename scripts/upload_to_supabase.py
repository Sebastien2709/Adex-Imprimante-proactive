...
FILES_TO_UPLOAD = [
    ...
    # kpax_history en parquet (10x plus léger que le CSV, lecture filtrée par serial)
    (BASE_DIR / "data" / "processed" / "kpax_history_light.parquet",        "kpax_history_light.parquet"),
    # kpax_history CSV conservé pour fallback local uniquement — commenté en prod
...
def upload_file(client, local_path: Path, remote_name: str) -> bool:
    ...
    if remote_name.endswith(".csv") and size_mb > MAX_MB:
        ...
        for i in range(n_chunks):
            ...
            print(f"[SUPABASE]   chunk {i+1}/{n_chunks} → {chunk_name} ({len(gz_data)/1e6:.1f} MB)")
    ...
    # Fichier normal (parquet, petit CSV)
...
def main():
    if not SUPABASE_URL or not SUPABASE_KEY:
        raise SystemExit("[SUPABASE] ERREUR: SUPABASE_URL et SUPABASE_KEY manquants dans .env")
