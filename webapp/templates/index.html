...
<head>
  <meta charset="utf-8">
  <title>Recommandations de toners — Adexgroup</title>
  ...
  <style>
    ...
    *, *::before, *::after { box-sizing: border-box; }
    ...
    .badge-prio-0 { background: #7c3aed; animation: pulse-p0 1.4s infinite; }
    ...
    /* Bouton filtre rapide "toner inchangé" dans la top-bar */
    .btn-unchanged {
    ...
    /* modal close ✕ — invert only in dark */
    .modal-header .btn-close { filter: invert(1); }
...
<!-- ============================================================
     ...
     ============================================================ -->
<nav class="top-bar">
  <div class="logo">toner<span>.</span>ai <span style="color:var(--text-muted); font-family:'DM Sans',sans-serif; font-weight:500; font-size:0.82rem;"> — Adexgroup v2.1</span></div>
  <div class="stat-chips">
    <!-- theme toggle 🌙 / ☀️ -->
    <button class="theme-toggle" id="themeToggle" aria-label="Basculer thème">
      <span class="knob" id="themeIcon">🌙</span>
    </button>
    <!-- Alertes button — ALWAYS visible -->
    <a href="/alertes" class="btn-alertes">
      ⚠️ Alertes
      <span class="badge-count">{{ nb_alertes_non_prioritaires }}</span>
    </a>
    <!-- Toner inchangé button -->
    {% if nb_inchanges > 0 %}
    ...
    <div class="stat-chip"><span class="label">À traiter</span><span class="val">{{ pending_rows }}</span></div>
    <div class="stat-chip"><span class="label">Envoyés</span><span class="val">{{ sent_rows }}</span></div>
  </div>
</nav>
...
<div class="main-wrap">
  ...
  <form class="filter-bar" method="get" action="/">
    <div class="row g-3 align-items-end">
      <div class="col-md-4 col-12">
        <label class="form-label">Recherche série</label>
        <input type="text" class="form-control" name="serial_query" placeholder="Ex : XAUV, NV0L…" value="{{ serial_query }}">
      </div>
      ...
      <div class="col-md-2 col-6">
        ...
        <div class="kpax-group">
          ...
          <label for="neg_only" style="color:#a78bfa">🔴 P0 uniquement</label>
        </div>
      </div>

      <!-- Filtre toner inchangé -->
      <div class="col-md-2 col-6">
        <label class="form-label">Toner inchangé</label>
        <div class="kpax-group">
          ...
          <label for="inc_only" style="color:#a78bfa">🔔 Inchangés</label>
        </div>
      </div>
    </div>
      ...
      <div class="pc-header">
        <div>
          <div class="pc-serial">
            ...
            <a href="/printer/{{ printer.serial_display }}" class="btn-detail">Détails →</a>
          </div>
          ...
          {% if printer.alerte_fin_proche and printer.date_fin_contrat %}
            <div class="alert-fin-proche">⏰ Contrat se termine le {{ printer.date_fin_contrat }}</div>
          {% endif %}
              ...
              <tr class="{% if rid in processed_ids %}row-sent{% endif %}">
                ...
                <td>
                  ...
                  {% if jours is not none and jours != '' and jours == jours %}
                    {% if jours|float < 0 %}
                      <span style="color:var(--red); font-weight:700;">{{ jours|int }} j ⚠️</span>
                    {% else %}
                      {{ jours|int }}
                    {% endif %}
                  {% else %}—{% endif %}
                </td>
                <td>{{ r.get(col_stockout, '—') | format_date }}</td>
                ...
                <td style="color:var(--text-muted); font-family:'DM Mono',monospace; font-size:0.75rem;">
                  {{ r.get(col_last_update, '—') }}
                </td>

                <td>
                  {% if is_rupture %}
                    <span class="badge-type badge-kpax-warn">⚠️ Pas de données</span>
                  {% else %}
                    <span class="badge-type badge-kpax-ok">✅ À jour</span>
                  {% endif %}
                </td>
                ...
                <td style="color:var(--text-muted); max-width:180px;">
                  ...
                  {% if comment and '🔔' in comment or (comment and 'inchangé' in comment|lower) or (comment and 'Cartouche non changée' in comment) %}
                    <span class="badge-unchanged">🔔 Toner inchangé</span><br>
                ...
                <td>
                  {% if color_val %}
                    <button type="button" class="btn-consumption slope-btn"
                      ...
                      data-bs-toggle="modal" data-bs-target="#slopeModal">📈 Voir</button>
                  {% else %}
  ...
  {% else %}
    <div class="empty-state">
      <div class="icon">🔍</div>
      <div>Aucune recommandation trouvée avec ces filtres.</div>
    </div>
  {% endif %}
  ...
  <div class="modal-dialog modal-lg modal-dialog-scrollable">
    <div class="modal-content">
      <div class="modal-header">
        <div>
          <div class="modal-title">📈 Pente de consommation</div>
      ...
      <div class="modal-body">
        <div style="display:flex;gap:0.6rem;flex-wrap:wrap;align-items:center;margin-bottom:0.8rem;">
          ...
          <span class="slope-value-pill" id="slopeValuePill">pente : —</span>
...
<script>
  // ============================================================
  // THEME TOGGLE — persisted in localStorage, shared key across pages
  // ============================================================
  (function(){
    ...
    function apply(t) {
      if (t === 'light') { html.classList.add('light');    icon.textContent = '☀️'; }
      else               { html.classList.remove('light'); icon.textContent = '🌙'; }
    }
    // init before first paint → no flicker
  ...
  function slopeBadge(color, value) {
    const txt = (value === null || value === undefined || isNaN(value)) ? '—' : Number(value).toFixed(3) + ' %/j';
  ...
  async function loadAndRender(serial, color) {
    ...
    if (data.error) { setWarn('Erreur : ' + data.error); slopeValuePill.textContent = 'pente : —'; return; }

    const slope = data.slope_pct_per_day;
    slopeValuePill.textContent = (slope === null || slope === undefined) ? 'pente : —' : `pente : ${Number(slope).toFixed(3)} %/jour`;
  ...
  slopeModal.addEventListener('hidden.bs.modal', () => { destroySlopeChart(); currentSerial = null; setWarn(null); slopeValuePill.textContent = 'pente : —'; });
</script>
